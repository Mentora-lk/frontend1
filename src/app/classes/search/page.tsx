    ...
    return (
      <Link href={`/classes/${course.id}`} style={{ textDecoration:'none' }}>
        <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)} //!track mouse on/leave
          style={{ display:'flex', background:palette.surface, borderRadius:18, overflow:'hidden', transition:'all 0.3s', transform:hov?'translateX(6px)':'translateX(0)', boxShadow:hov?`0 12px 32px ${sc}18`:'0 2px 12px rgba(0,0,0,0.05)', border:hov?`1px solid ${sc}30`:'1px solid rgba(0,0,0,0.04)' }}>
          <div style={{ width:5, background:sc, flexShrink:0 }}/>
        {/* course img */}
          <div style={{ width:180, flexShrink:0, overflow:'hidden' }}>
           <img src={thumbnail} alt={course.title} style={{ width:'100%', height:'100%', objectFit:'cover', transition:'transform 0.5s', transform:hov?'scale(1.06)':'scale(1)' }}/>
          </div>
          ...
          <div style={{ flex:1, padding:'18px 22px', display:'flex', flexDirection:'column', justifyContent:'space-between' }}>
            ...
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:8 }}>
              <div style={{ display:'flex', gap:10 }}>
                <span style={{ fontSize:11, color:palette.textMuted }}>📍 {course.location}</span>
  ...
  return (
    <Link href={`/classes/${course.id}`} style={{ textDecoration:'none' }}>
      <div onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
        style={{ background:palette.surface, borderRadius:20, overflow:'hidden', transition:'all 0.32s cubic-bezier(.22,1,.36,1)', transform:hov?'translateY(-8px)':'translateY(0)', boxShadow:hov?`0 20px 44px ${sc}20`:'0 4px 18px rgba(0,0,0,0.06)', border:hov?`1px solid ${sc}30`:'1px solid rgba(0,0,0,0.04)' }}>
        ...
        <div ref={topRef} style={{ maxWidth:1380, margin:'0 auto', padding:'28px 6% 64px', display:'flex', gap:26, alignItems:'flex-start' }}>
          ...
          <div className="filter-sidebar" style={{ width:240, flexShrink:0 }}>
            <div className="filter-card">
              ...
              <div style={{ marginBottom:20 }}>
                ...
                <select value={subject} onChange={e=>setSubject(e.target.value)} style={{ width:'100%', border:`1.5px solid ${palette.border}`, borderRadius:10, padding:'9px 12px', fontSize:13, color:palette.textSecondary, background:palette.surface, fontFamily:"'DM Sans',sans-serif", cursor:'pointer' }}>
              ...
              <div style={{ marginBottom:20 }}>
                ...
                {['All','online','offline','both'].map(m=>(
                  <label key={m} style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8, cursor:'pointer', fontSize:13, color:palette.textSecondary }}>
              ...
              <div style={{ marginBottom:20 }}>
                ...
                <select value={location} onChange={e=>setLocation(e.target.value)} style={{ width:'100%', border:`1.5px solid ${palette.border}`, borderRadius:10, padding:'9px 12px', fontSize:13, color:palette.textSecondary, background:palette.surface, fontFamily:"'DM Sans',sans-serif", cursor:'pointer' }}>
          ...
          <div style={{ flex:1, minWidth:0 }}>
            ...
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16, flexWrap:'wrap', gap:12 }}>
              <span style={{ fontFamily:"'Playfair Display',serif", fontSize:20, fontWeight:700, color:palette.textPrimary }}>
                {total} <span style={{ fontWeight:400, fontSize:16, color:palette.textSecondary }}>classes found</span>
                {query && <span style={{ fontSize:13, color:palette.textMuted, marginLeft:10 }}>for &quot;<span style={{ color:'#10B981', fontWeight:600 }}>{query}</span>&quot;</span>}
              </span>
              <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                <select value={sortBy} onChange={e=>setSortBy(e.target.value)} style={{ padding:'8px 28px 8px 12px', borderRadius:10, fontSize:12, fontWeight:500, border:`1.5px solid ${palette.border}`, background:palette.surface, color:palette.textSecondary, fontFamily:"'DM Sans',sans-serif", cursor:'pointer' }}>
            ...
            {loading ? (
              <div style={{ textAlign:'center', padding:'60px 0' }}>
                ...
                <p style={{ color:palette.textMuted, fontSize:14 }}>Loading courses...</p>
              </div>
            ) : error ? (
              ...
              <>
                ...
                <p style={{ textAlign:'center', fontSize:12, color:palette.textMuted, marginTop:12 }}>
                  Showing {(page-1)*6+1}–{Math.min(page*6,total)} of {total} results
                </p>
              </>
            ) : (
              <div style={{ textAlign:'center', padding:'80px 24px', background:palette.surface, borderRadius:20, border:`1px solid ${palette.border}` }}>
                <div style={{ fontSize:52, marginBottom:14 }}>🔍</div>
                <h3 style={{ fontFamily:"'Playfair Display',serif", fontSize:22, fontWeight:700, marginBottom:10 }}>No classes found</h3>
                <p style={{ fontSize:14, color:palette.textMuted, marginBottom:22 }}>Try adjusting your filters.</p>
