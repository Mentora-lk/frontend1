...
function FieldLabel({ label, required, hint, children }: { label:string; required?:boolean; hint?:string; children:React.ReactNode }) {
  ...
  return <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
    <label style={{ fontSize:13, fontWeight:600, color:palette.textSecondary, display:'flex', alignItems:'center', gap:4 }}>
      {label}{required && <span style={{ color:'#EF4444' }}>*</span>}
    </label>
    {children}
    {hint && <span style={{ fontSize:11, color:palette.textMuted }}>{hint}</span>}
  </div>;
}
function NavBtn({ label, onClick, primary=false, icon }: { label:string; onClick:()=>void; primary?:boolean; icon?:React.ReactNode }) {
  const palette = usePalette();
  return <button onClick={onClick} style={{
    background: primary ? 'linear-gradient(135deg,#10B981,#059669)' : palette.surface,
    color: primary ? 'white' : palette.textSecondary,
...
function StepIndicator({ currentStep }: { currentStep: number }) {
  ...
  return (
    <div style={{ background:palette.surface, borderRadius:18, padding:'22px 32px', boxShadow:palette.shadow, border:`1px solid ${palette.border}` }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'center' }}>
        {steps.map((s,i)=>(
          <div key={s.n} style={{ display:'flex', alignItems:'center', flex: i<2?1:'none' }}>
            ...
            {i<2 && <div style={{ flex:1, height:2, margin:'0 4px', marginBottom:22, background:currentStep>s.n?'#10B981':palette.border, transition:'background 0.4s' }}/>}
          </div>
        ))}
      </div>
    </div>
...
export default function EnrollPage() {
  ...
  const inputStyle = (hasErr?: boolean): React.CSSProperties => ({
    ...
    fontFamily:"'DM Sans',sans-serif", color:palette.textPrimary,
    border:`1.5px solid ${hasErr?'#FCA5A5':palette.border}`,
    background:hasErr?'#FFF5F5':palette.surface, outline:'none', transition:'border-color 0.2s',
  });
    ...
    <>
      ...
      <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#F4F6F5' }}>
        <div style={{ textAlign:'center' }}>
          ...
          <p style={{ color:palette.textMuted, fontSize:15 }}>Loading enrollment form...</p>
        </div>
      </div>
    </>
  );
        ...
        <div style={{ maxWidth:1280, margin:'0 auto', padding:'32px 6% 80px', display:'flex', gap:28, alignItems:'flex-start' }}>
          ...
          <div style={{ flex:1, minWidth:0, display:'flex', flexDirection:'column', gap:20 }}>
              ...
              <div className="step-panel" style={{ background:palette.surface, borderRadius:22, padding:'32px', boxShadow:palette.shadow, border:`1px solid ${palette.border}` }}>
                <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:28 }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize:13, color:palette.textMuted }}>Tell us a bit about yourself</p>
                  </div>
                </div>
              ...
              <div className="step-panel">
                ...
                {course?.mode==='both' && (
                  <div style={{ background:palette.surface, borderRadius:22, padding:'28px 32px', marginBottom:20, boxShadow:palette.shadow, border:`1px solid ${palette.border}` }}>
                    ...
                    <p style={{ fontSize:13, color:palette.textMuted, marginBottom:20 }}>This class supports both — choose your preference</p>
                    <div style={{ display:'flex', gap:14 }}>
                      {[
                        ...
                        <div key={m.val} onClick={()=>setPreferredMode(m.val)} style={{ flex:1, border:`2px solid ${preferredMode===m.val?m.color:'#E5E7EB'}`, borderRadius:14, padding:20, cursor:'pointer', textAlign:'center', transition:'all 0.22s', background:preferredMode===m.val?(m.val==='online'?'#f0fdf4':'#eff6ff'):'white' }}>
                          ...
                          <div style={{ fontWeight:700, fontSize:15, color:preferredMode===m.val?m.color:palette.textPrimary, marginBottom:4 }}>{m.label}</div>
            ...
            {step===3 && (
              <div className="step-panel">
                <div style={{ background:palette.surface, borderRadius:22, padding:'28px 32px', marginBottom:20, boxShadow:palette.shadow, border:`1px solid ${palette.border}` }}>
                  <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:22, fontWeight:700, color:palette.textPrimary, marginBottom:20, display:'flex', alignItems:'center', gap:10 }}>
                    <span style={{ width:4, height:22, background:'#10B981', borderRadius:2, display:'inline-block' }}/>Review Your Enrollment
                  </h2>

                  {/* Personal summary */}
                  <div style={{ background:palette.surfaceAlt, borderRadius:14, padding:'18px 20px', marginBottom:16 }}>
                    ...
                    {[{l:'Full Name',v:fullName},{l:'Email',v:email},{l:'Phone',v:phone},{l:'School',v:school||'—'},{l:'Grade',v:grade},...(message?[{l:'Message',v:message}]:[])].map(row=>(
                      <div key={row.l} style={{ display:'flex', gap:12, padding:'10px 0', borderBottom:`1px solid ${palette.border}` }}>
                        <span style={{ fontSize:13, color:palette.textSecondary, width:120, flexShrink:0 }}>{row.l}</span>
