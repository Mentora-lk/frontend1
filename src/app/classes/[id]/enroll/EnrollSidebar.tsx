...
export default function EnrollSidebar({
  ...
  return (
    <div style={{ position: 'sticky', top: 84 }}>
      ...
      <div style={{ background: palette.surface, borderRadius: 20, overflow: 'hidden', boxShadow: '0 8px 32px rgba(0,0,0,0.1)', border: '1px solid rgba(0,0,0,0.05)', marginBottom: 16 }}>
        ...
        <div style={{ padding: '16px 18px' }}>
          ...
          <Link href={`/classes/${courseId}`}>
            <button style={{ width: '100%', marginTop: 14, background: 'none', border: `1.5px solid ${palette.border}`, borderRadius: 10, padding: '9px', fontSize: 12, fontWeight: 600, color: palette.textSecondary, cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", transition: 'all 0.2s' }}
      ...
      {/* Step tracker */}
      {currentStep < 4 && (
        <div style={{ background: palette.surface, borderRadius: 16, padding: '18px 20px', boxShadow: palette.shadow, border: `1px solid ${palette.border}` }}>
          <p style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: palette.textMuted, marginBottom: 14 }}>Your Progress</p>
          {[{ n: 1, label: 'Personal Details' }, { n: 2, label: 'Pick Schedule' }, { n: 3, label: 'Confirm & Submit' }].map((s, i) => (
            <div key={s.n} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: i < 2 ? `1px solid ${palette.border}` : 'none' }}>
              ...
              <span style={{ fontSize: 13, fontWeight: currentStep === s.n ? 700 : 400, color: currentStep === s.n ? '#10B981' : currentStep > s.n ? '#059669' : palette.textMuted, transition: 'color 0.3s' }}>
                {s.label}
              </span>
