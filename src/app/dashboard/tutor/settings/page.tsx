...
const Row = ({ label, desc, on, onToggle }: { label:string; desc:string; on:boolean; onToggle:()=>void }) => {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'16px 0', borderBottom:`1px solid ${palette.border}` }}>
      <div>
        <p style={{ fontSize:14, fontWeight:600, color:palette.textPrimary, marginBottom:2 }}>{label}</p>
        <p style={{ fontSize:12, color:palette.textMuted }}>{desc}</p>
      </div>
...
const Section = ({ title, icon, children, onSave, saved }: { title:string; icon:string; children:React.ReactNode; onSave:()=>void; saved?: string }) => {
  const palette = usePalette();
  return (
...
export default function TutorSettingsPage() {
  ...
  return (
    <TutorDashboardLayout title="Settings" subtitle="Manage your account preferences and notifications.">
      ...
      {/* Account */}
      <Section title="Account Security" icon="🔐" onSave={() => save('Account Security')} saved={saved}>
      ...
      {/* Notifications */}
      <Section title="Notifications" icon="🔔" onSave={() => save('Notifications')} saved={saved}>
      ...
      {/* Privacy */}
      <Section title="Privacy" icon="🔒" onSave={() => save('Privacy')} saved={saved}>
      ...
      <div style={{ background:palette.surface, borderRadius:20, padding:'24px 28px', boxShadow:palette.shadow, border:'1.5px solid #FECACA' }}>
        ...
        <p style={{ fontSize:13, color:palette.textSecondary, marginBottom:18, lineHeight:1.6 }}>These actions are irreversible. Please proceed with caution.</p>
