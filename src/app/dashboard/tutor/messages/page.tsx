  ...
  return (
    <TutorDashboardLayout title="Messages" subtitle="Chat with your students.">
      <div style={{ display:'flex', gap:20, height:580 }}>
        ...
        {/* Sidebar */}
        <div style={{ width:280, background:'white', borderRadius:18, boxShadow:'0 4px 20px rgba(0,0,0,0.06)', border:'1px solid rgba(0,0,0,0.04)', overflow:'hidden', flexShrink:0, display:'flex', flexDirection:'column' }}>
          <div style={{ padding:'18px 18px 12px', borderBottom:'1px solid #F3F4F6' }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, background:'#F9FAFB', borderRadius:10, padding:'8px 12px' }}>
              ...
              <input placeholder="Search messages..." style={{ border:'none', outline:'none', background:'transparent', fontSize:13, color:'#374151', width:'100%', fontFamily:"'DM Sans',sans-serif" }} />
            </div>
          </div>
          <div style={{ flex:1, overflowY:'auto' }}>
            {CONVERSATIONS.map(c => (
              <div key={c.id} onClick={() => setActiveId(c.id)} style={{ display:'flex', alignItems:'center', gap:12, padding:'14px 18px', cursor:'pointer', background:activeId===c.id?'#ECFDF5':'white', borderLeft:`3px solid ${activeId===c.id?'#10B981':'transparent'}`, transition:'all 0.2s' }}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <p style={{ fontSize:12, color:'#9CA3AF', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{c.lastMsg}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Chat area */}
        <div style={{ flex:1, background:'white', borderRadius:18, boxShadow:'0 4px 20px rgba(0,0,0,0.06)', border:'1px solid rgba(0,0,0,0.04)', display:'flex', flexDirection:'column', overflow:'hidden' }}>
          {/* Chat header */}
          ...
          <div style={{ flex:1, overflowY:'auto', padding:'20px 22px', display:'flex', flexDirection:'column', gap:12 }}>
            {msgs.map((m, i) => (
              <div key={i} style={{ display:'flex', justifyContent:m.from==='me'?'flex-end':'flex-start' }}>
                <div style={{ maxWidth:'70%', background:m.from==='me'?'linear-gradient(135deg,#10B981,#059669)':'#F3F4F6', color:m.from==='me'?'white':'#111827', borderRadius:m.from==='me'?'18px 18px 4px 18px':'18px 18px 18px 4px', padding:'11px 16px', fontSize:14, lineHeight:1.5 }}>
                  <p>{m.text}</p>
          ...
          <div style={{ padding:'14px 18px', borderTop:'1px solid #F3F4F6', display:'flex', gap:10 }}>
            <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key==='Enter' && sendMsg()} placeholder="Type a message..." style={{ flex:1, border:'1.5px solid #E5E7EB', borderRadius:12, padding:'11px 16px', fontSize:14, fontFamily:"'DM Sans',sans-serif", outline:'none', color:'#374151' }} />
