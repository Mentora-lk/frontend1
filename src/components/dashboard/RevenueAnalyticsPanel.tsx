...
// Revenue Analytics content, extracted out of its own standalone page so it
// can also be embedded as a tab inside the Profile page. Self-contained —
// fetches its own data and manages its own state, same as it did as a page.
export default function RevenueAnalyticsPanel() {
  ...
  if (loading) {
    return <div style={{ display: 'flex', justifyContent: 'center', padding: '40px', color: palette.textMuted }}>Loading...</div>;
  }

  if (error || !data) {
    return <div style={{ textAlign: 'center', padding: '60px 0', color: palette.textMuted }}>{error || 'No data available.'}</div>;
  }
